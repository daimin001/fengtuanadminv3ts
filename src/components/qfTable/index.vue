...
<template>
  <el-table :data="data" :border="border" :row-key="rowKey" class="qfTable">
        ...
        <template #default="scope">
          ...
          <template v-if="col.type === 'tag'">
            <el-tag
              :type="
                col.payload!.render
                  ? col.payload!.render(scope.row[col.payload!.content])?.type
                  : col.payload!.type
              "
            >
              {{
                col.payload!.render
                  ? col.payload!.render(scope.row[col.payload!.content])?.msg
                  : col.payload!.content
              }}
            </el-tag>
          </template>
          ...
          <!-- img -->
          <template v-if="col.type === 'imgs'">
            <img
              v-for="(imgItem, imgI) in scope.row[col.payload!.src]"
              :key="imgI"
              :src="imgItem"
              :alt="col.payload!.alt"
              :width="col.payload!.width"
