...
<template>
	<el-form :inline="inline" :size="inline ? 'default' : 'large'" :label-width="labelWidth" :model="formData" ref="formRef">
		<!-- 动态生成选项 -->
			...
			>
				...
				<template v-if="item.type === 'select'">
					<el-select v-model="formData[item.field]" :placeholder="item.placeholder" :disabled="item.disabled">
						<el-option v-for="(tmp, tmpI) in item.options" :key="tmpI" :label="tmp.label" :value="tmp.value"> </el-option>
					</el-select>
				</template>
				<!-- select -->
